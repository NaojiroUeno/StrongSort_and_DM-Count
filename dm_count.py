...
def detect_crowd(image):
    ...
    for label in unique_labels:
        ...
        if len(cluster_points) > 2:
            ...
            # 凸包を描画
            hull_points = hull_points.reshape((-1, 1, 2))  # OpenCVの形式に変換
            cv2.polylines(image, [hull_points], isClosed=True, color=(255, 0, 0), thickness=2)

            # 重心を描画
            cluster_center = np.mean(cluster_points, axis=0)[::-1]  # (y, x)から(x, y)に変換
            cv2.circle(image, center=(int(cluster_center[0]), int(cluster_center[1])),
